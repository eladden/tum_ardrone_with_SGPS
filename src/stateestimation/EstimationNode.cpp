...
EstimationNode::EstimationNode()
{
    ...
    //sgps_channel = nh_.resolveName("/fix");
    sgps_channel = nh_.resolveName("ground_truth/state");
	...
	sgps_sub 	 = nh_.subscribe(sgps_channel, 10, &EstimationNode::sgpsCb, this );
	//altm_sub 	 = nh_.subscribe(altm_channel, 1000, &EstimationNode::altmCb, this );
...
//void EstimationNode::sgpsCb(const sensor_msgs::NavSatFix::ConstPtr sgps)
//{
//    double fixlon =sgps->longitude;
//    fixlon = fixlon * M_PI /180;
//    double fixlat = sgps->latitude;
//    fixlat = fixlat * M_PI /180;
//    double fixalt = sgps->altitude;
    
//    filter->fix_lon = fixlon;
//    filter->fix_lat = fixlat;
//    filter->fix_alt = fixalt;
//    double cosLat_ = cos(fixlat);
//    double cosLon_ = cos(fixlon);
//    double sinLat_ = sin(fixlat);
//    double sinLon_ = sin(fixlon);

//    filter->C_4GPS = 1.0/sqrt(cosLat_ * cosLat_ + (1 - f_const) * (1 - f_const) * sinLat_ * sinLat_);
    //filter->S_4GPS = (1.0 - f_const) * (1.0 - f_const) * C_4GPS;

//    filter->fix_lon = fixlon;
//    filter->fix_lat = fixlat;
//    filter->fix_alt = fixalt;
//    filter->cosLat = cosLat_;
//    filter->cosLon = cosLon_;
//    filter->sinLat = sinLat_;
//    filter->sinLon = sinLon_;
//}
void EstimationNode::sgpsCb(const nav_msgs::Odometry::ConstPtr& sgps)
{
	filter->x_GPS_obs = -sgps->pose.pose.position.y;
	filter->y_GPS_obs = sgps->pose.pose.position.x;
	filter->z_GPS_obs = sgps->pose.pose.position.z;
}

//void altmCb(const cvg_sim_msgs::Altimeter::altitude altitude);
//{
//    double altm = altitude/1000; //in mm to comply with IMU measurments
//}
...
void EstimationNode::Loop()
{
	  ...
	  while (nh_.ok())
	  {
		  ...
		  //ros::Publisher UFGPS_pub = nh_.advertise<geometry_msgs::Point>("UFGPS", 10); ////////<-------debug
		  ...
		  // if you're using simulated GPS add an obselvation
		  //if(filter->useSGPS)
		  //	filter->addSGPSObservation(filter->getCurrentPose(), getMS(ros::Time().now()));
		  
		  //positionMsg.x = filter->x_GPS_obs;
		  //positionMsg.y = filter->y_GPS_obs;
		  //positionMsg.z = filter->z_GPS_obs;
