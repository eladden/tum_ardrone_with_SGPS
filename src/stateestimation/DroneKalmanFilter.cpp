...
const double varPoseObservation_xy_GPS = 0.01*0.01;
const double varAccelerationError_xy = 8*8;

const double varPoseObservation_z_PTAM = 0.08*0.08;
const double varPoseObservation_z_GPS = 0.05*0.05;
...
void DroneKalmanFilter::reset()
{
	...
	// set baseline fore GPS
	//baselineX_SGPS = (earth_Rad * C_4GPS + fix_alt) * cosLat * cosLon;
    	//baselineY_SGPS = (earth_Rad * C_4GPS + fix_alt) * cosLat * sinLon;
   	//baselineZ_SGPS = (earth_Rad * C_4GPS + fix_alt) * sinLat;
...
void DroneKalmanFilter::observeIMU_XYZ(const ardrone_autonomy::Navdata* nav)
{
	...
	if (this->useSGPS)
	{
		navaltd = z_GPS_obs*1000;//((earth_Rad * C_4GPS + fix_alt) * sinLat - baselineZ_SGPS)*1000; // fix_alt*1000;//*0.001;
	}
	else
	{	
		navaltd = nav->altd;
	...
	if(last_z_IMU != navaltd || nav->header.seq - last_z_packageID > 8)
	{
		...
		if(lastPosesValid)
		{
			...
			double imuHeightDiff = (navaltd - baselineZ_IMU )*0.001;	// TODO negative heights??
		...
		{
			double imuHeightDiff = (navaltd - baselineZ_IMU )*0.001;
...
//void DroneKalmanFilter::observeSGPS(TooN::Vector<6> pose)
void DroneKalmanFilter::observeSGPS(void)
{
	//double x_GPS_obs = (earth_Rad * C_4GPS + fix_alt) * cosLat * cosLon;
    	//double y_GPS_obs = (earth_Rad * C_4GPS + fix_alt) * cosLat * sinLon;
   	//double z_GPS_obs = (earth_Rad * C_4GPS + fix_alt) * sinLat;

	//x.observePose(x_GPS_obs-baselineX_SGPS,varPoseObservation_xy_GPS);
	//y.observePose(y_GPS_obs-baselineY_SGPS,varPoseObservation_xy_GPS);
	//z.observePose(z_GPS_obs-baselineZ_SGPS,varPoseObservation_z_GPS);
	x.observePose(x_GPS_obs,varPoseObservation_xy_GPS);
	y.observePose(y_GPS_obs,varPoseObservation_xy_GPS);
	z.observePose(z_GPS_obs,varPoseObservation_xy_GPS);
...
//void DroneKalmanFilter::addSGPSObservation(TooN::Vector<6> trans, int time)
//{
//	if(time > predictdUpToTimestamp)
//		predictUpTo(time, true,true);
